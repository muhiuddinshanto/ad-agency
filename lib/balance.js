...
export async function recalculateBalance(clientId) {
  ...
  // 2. Running Spend (Sum of each campaign's actual or overridden spend)
  let totalSpent = 0;
  for (const campaign of campaigns) {
    let campaignSpent = 0;
    if (campaign.manualSpendOverride !== undefined && campaign.manualSpendOverride !== null && campaign.manualSpendOverride >= 0) {
      campaignSpent = campaign.manualSpendOverride;
    } else {
      const campaignSpends = dailySpends.filter(ds => ds.campaign.toString() === campaign._id.toString());
      campaignSpent = campaignSpends.reduce((sum, ds) => sum + (ds.amount || 0), 0);
    }

    // Sync individual campaign's runningSpend
    if (campaign.runningSpend !== campaignSpent) {
      campaign.runningSpend = campaignSpent;
      await campaign.save();
    }
